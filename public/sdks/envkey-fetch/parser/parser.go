...
import (
	...
	"github.com/envkey/go-envkeyfetch/trust"
...
func (response *FetchResponse) parseKeys(encryptionKey string) (*ResponseWithKeys, error) {
	...
	json.Unmarshal(decryptedPrivkeyBytes, &decryptedPrivkey)
	if err != nil {
		return nil, err
	}
...
func (response *DecryptedResponse) toJson() (string, error) {
	resMap, err := response.toMap()
	if err != nil {
		return "", err
	}
	envJson, err := json.Marshal(resMap)
	if err != nil {
		return "", err
	}
...
func (response *DecryptedResponse) toMap() (map[string]interface{}, error) {
	env := make(map[string]interface{})
	localsOrSubEnvs := make(map[string]interface{}) // either subenv or locals
...
func (blob *DecryptedKeyableBlob) toMaps() (map[string]interface{}, map[string]interface{}, error) {
	env := make(map[string]interface{})
