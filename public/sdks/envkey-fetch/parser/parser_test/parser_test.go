...
import (
	...
	"github.com/envkey/go-envkeyfetch/crypto"
	"github.com/envkey/go-envkeyfetch/parser"
...
func TestParse(t *testing.T) {
	...
	envJson, _, _, _, err = responseWithLocalsBlocksInheritance.Parse(encryptionKeyString)
	assert.Nil(t, err, "Should not return an error.")
	...
	envJson, _, _, _, err = responseWithSubEnvsBlocksInheritance.Parse(encryptionKeyString)
	assert.Nil(t, err, "Should not return an error.")
	...
	envJson, _, newSignedTrustedRoot, replacementIds, err := responseWithSingleRootPubkeyReplacement.Parse(encryptionKeyString)
	assert.Nil(t, err, "Should not return an error.")
	...
	envJson, _, newSignedTrustedRoot, replacementIds, err = responseWithMultiRootPubkeyReplacements.Parse(encryptionKeyString)
	assert.Nil(t, err, "Should not return an error.")
	assert.NotNil(t, newSignedTrustedRoot, "Should return a new signed trusted root to send back to server")
