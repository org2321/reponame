...
def fetch_env(key, cache_enabled=False):
  path = __lib_path()
  args = [path, key, "--client-name", "envkey-python", "--client-version", pkg_resources.get_distribution(os.environ.get("ENVKEY_DEV_OVERRIDE_PACKAGE_NAME") or "envkey").version]
  if cache_enabled:
    args.append("--cache")
