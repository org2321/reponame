...
function fetch(keyOrCbOrOpts, optsOrCb, maybeCb){
  ...
  var ext = platformPart == "windows" ? ".exe" : "",
      filePath = path.join(__dirname, "ext", ["envkey-fetch", platformPart, archPart].join("_"), ("envkey-fetch" + ext)),
      execArgs = [key, (isDev ? "--cache" : ""), "--client-name", "envkey-node", "--client-version", packageJson.version]
