...
const SUB_ENV_PERMISSIONS_BY_ACCESS_LEVEL = {
  none: [],
  read_meta_only: ["read_subenvs_inherits", "read_subenvs_meta"],
  read_only: ["read_subenvs_inherits", "read_subenvs_meta", "read_subenvs"],
  read_write: [
    ...
    return (
      <div>
            ...
            <div className="select">
              <select
                ...
                onChange={(e) => {
                  ...
                  if (
                    subPermissions.includes("read_subenvs") &&
                    !subPermissions.includes("read_subenvs_history")
                  ) {
        ...
        {baseAccessLevel == "none" ? (
          ...
          <div>
            <div className="field">
              <label>Sub-Environment Permissions</label>
              <div className="select">
                <select
                  disabled={appRole.hasFullEnvironmentPermissions || submitting}
                  onChange={(e) => {
                    const selectedLevel = e.target.value as AccessLevel;
                    const basePermissions = BASE_ENV_PERMISSIONS_BY_ACCESS_LEVEL[
                      baseAccessLevel
                    ] as Rbac.EnvironmentPermission[];
                    ...
                    if (
                      ...
                      subPermissions.push("read_subenvs_history");
                    }
            ...
            {permissionsSet.has("read_subenvs") &&
            !permissionsSet.has("write_subenvs") ? (
              <div
                className={
                  "field checkbox" +
                  (permissionsSet.has("read_subenvs_history")
                    ? " selected"
                    : "") +
                  (submitting ? " disabled" : "")
                }
                onClick={() => {
                  setPermissionsByAppRoleIdState({
                    ...permissionsByAppRoleIdState,
                    [appRole.id]: permissionsSet.has("read_subenvs_history")
                      ? R.without(["read_subenvs_history"], permissions)
                      : [...permissions, "read_subenvs_history"],
                  });
                }}
              >
                ...
                <input
                  type="checkbox"
                  checked={permissionsSet.has("read_subenvs_history")}
                />
              </div>
            ) : (
              ""
