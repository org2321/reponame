...
type FilterLogType = typeof BASE_LOG_TYPE_OPTIONS[number]["value"];
...
type LogManagerState = Pick<Logs.FetchLogParams, "sortDesc" | "ips"> & {
...
export const LogManager: OrgComponent<RouteProps> = (props) => {
  ...
  useEffect(() => {
    return () => {
      props.dispatch({ type: Client.ActionType.CLEAR_LOGS });
    };
  }, []);
    ...
    if (logManagerState.environmentRoleOrCompositeIds) {
      ...
      if (localsSelected) {
        ...
        if (
          ...
          envParentIds = [logParent.id];
        } else if (logManagerState.envParentIds) {
          envParentIds = logManagerState.envParentIds;
        } else {
          envParentIds = [
            ...byTypeWithDeleted.apps,
            ...byTypeWithDeleted.blocks,
          ].map(R.prop("id"));
        }

        let localsUserIds: string[];
        if (
          logParent &&
          (logParent.type == "orgUser" || logParent.type == "cliUser")
        ) {
          localsUserIds = [logParent.id];
        } else if (logManagerState.userIds) {
          localsUserIds = logManagerState.userIds;
        } else {
          localsUserIds = [
            ...byTypeWithDeleted.orgUsers,
            ...byTypeWithDeleted.cliUsers,
          ].map(R.prop("id"));
        }

        targetIds = targetIds.concat(localsUserIds);
      }
  ...
  const subEnvironmentOptionsFn = (
    ...
    subEnvironments.length > 0
      ? [
          {
            label: (deleted ? "Deleted " : "") + "Sub-Environments",
      ...
      <div className="transaction">
        <div className="transaction-summary">
          <div>
            ...
            <span className="date">
              <label>When</label>
              {moment(logged.createdAt).format("MMM D YYYY h:mm:ss.SSS A")}
            </span>
          </div>
