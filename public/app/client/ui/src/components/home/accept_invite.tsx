...
export const AcceptInvite: Component = (props) => {
  ...
  const [needsExternalAuthError, setNeedsExternalAuthError] =
    useState<Api.Net.RequiresExternalAuthResult | undefined>(undefined);
  const [externalAuthErrorMessage, setExternalAuthErrorMessage] =
  ...
  const renderButtons = () => {
    ...
    if (isLoading) {
      ...
      label = "Waiting for external auth...";
    } else if (isAccepting) {
      label = "Signing In...";
