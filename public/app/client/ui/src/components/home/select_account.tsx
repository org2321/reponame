      ...
      <div key={`deployment-${i}`} className="account">
        <span className="remove">
          <a
            onClick={async (e) => {
              e.preventDefault();
              // TODO: destroy self-hosted instance? probably force them to use the CLI. destroyHost(). we also need to store the profile and region.
              const conf = await confirm(
                "Really clear the pending deployment? AWS resources will still be consumed until you run the CLI tool `envkey host destroy`!"
              );
              if (!conf) {
                return;
              }
              setDestroyingDeployment(pendingDeployment);
              await dispatch({
                type: Client.ActionType.CLEAR_PENDING_SELF_HOSTED_DEPLOYMENT,
                payload: { deploymentTag: pendingDeployment.deploymentTag },
              });
              setDestroyingDeployment(undefined);
            }}
            className="exit"
          >
