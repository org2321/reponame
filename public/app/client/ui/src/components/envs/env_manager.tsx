...
export const EnvManager: OrgComponent<EnvManagerRouteProps> = (props) => {
    ...
    () =>
      Boolean(
        (props.ui.envManager.editingEnvironmentId &&
          isMultiline(props.ui.envManager.editingInputVal ?? "")) ||
          (props.ui.envManager.entryForm.editingEnvironmentId &&
            isMultiline(
  ...
  const showLeftNav =
    environmentIds.length > 1 && props.ui.envManager.environmentStartIndex > 0;
  ...
  const tentativeMaxValCols = isSub
    ? 1
    : Math.max(1, Math.floor(tentativeValColsWidth / layout.ENV_MIN_COL_WIDTH));
  ...
  const envManagerProps: EnvManagerProps = {
    ...
    showLeftNav,
    showRightNav,
    allEnvironmentIds: environmentIds,
  ...
  if (searchParams.get("importEnvironmentId")) {
  ...
  return (
    <div
      className={
        ...
        " " +
        style({
          paddingBottom: props.ui.pendingFooterHeight,
        })
      }
    >
      {contents}
