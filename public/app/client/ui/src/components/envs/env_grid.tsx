...
import * as EnvCell from "./env_cell";
...
> = (props) => {
  const {
    showLeftNav,
    showRightNav,
    core: { graph, graphUpdatedAt, pendingEnvUpdates },
  } = props;
  ...
  const renderRow = (entryKey: string, i: number) => {
    ...
    return props.ui.envManager.confirmingDeleteEntryKeyComposite ==
      [props.envParentId, entryKey].join("|") ? (
  ...
  const displayEntryKeys = useMemo(() => {
    if (props.editingMultiline && props.ui.envManager.editingEntryKey) {
    ...
    props.editingMultiline && props.ui.envManager.editingEntryKey,
    filter,
  ]);
  ...
  const committedEntryKeysSet = useMemo(
    ...
    [
      props.core.graphUpdatedAt,
      currentUserId,
      JSON.stringify(entryKeyEnvironmentIds),
    ]
  );
