...
export const checkUpgradesAvailableLoop = async (store: Client.ReduxStore) => {
  ...
  try {
    await checkSelfHostedUpgradesAvailable(store);
  } catch (err) {
    log("Error checking self-hosted upgrades available:", { err });
  }

  setTimeout(() => checkUpgradesAvailableLoop(store), CHECK_UPGRADES_INTERVAL);
};

const checkSelfHostedUpgradesAvailable = async (store: Client.ReduxStore) => {
  let procState = store.getState();
  ...
  for (let account of Object.values(procState.orgUserAccounts)) {
    ...
    const org = g.getOrg(accountState.graph);
