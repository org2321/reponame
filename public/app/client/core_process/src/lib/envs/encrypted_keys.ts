...
import { Client, Api, Model, Crypto, Blob } from "@core/types";
import {
  graphTypes,
...
import { encrypt, secureRandomAlphanumeric } from "@core/lib/crypto";
...
export const encryptedKeyParamsForEnvironments = async (params: {
  ...
  newChangesetKeysByEnvironmentId: Record<string, string>;
  reencryptChangesetKeysById: Record<string, string>;
  ...
  const newChangesetKeysByEnvironmentId: Record<string, string> = {};
  const reencryptChangesetKeysById: Record<string, string> = {};
  ...
  const inheritingEnvironmentIdsByEnvironmentId: Record<string, Set<string>> =
    {};
  ...
  for (let environmentId of environmentIds) {
    ...
    let newChangesetsSymmetricKey: string | undefined;
    ...
    if (pending && !reencryptChangesets) {
      newChangesetsSymmetricKey = getNewSymmetricEncryptionKey();
      newChangesetKeysByEnvironmentId[environmentId] =
        newChangesetsSymmetricKey;
    }

    let changesetsToReencrypt: Client.Env.Changeset[] | undefined;
    if (reencryptChangesets) {
      changesetsToReencrypt = (state.changesets[environmentId] ?? []).flatMap(
        R.prop("changesets")
      );
      for (let changeset of changesetsToReencrypt) {
        const symmetricKey = getNewSymmetricEncryptionKey();
        reencryptChangesetKeysById[changeset.id] = symmetricKey;
      }
    }
    ...
    for (let userId of allUserIds) {
      ...
      if (localsUserId) {
        if (
          !(envSymmetricKey || metaSymmetricKey || newChangesetsSymmetricKey)
        ) {
          continue;
        }
            ...
            if (
              ...
              if (
                ...
                toEncrypt.push([
                  ...
                  {
                    data: newChangesetsSymmetricKey,
                    pubkey,
                    privkey,
                  },
                ]);
                ...
                for (let changeset of changesetsToReencrypt) {
                  ...
                  toEncrypt.push([
                    [...path, "data"],
                    {
                      data: reencryptChangesetKeysById[changeset.id],
                      pubkey,
                      privkey,
                    },
                  ]);
        ...
        for (let deviceId of deviceIds) {
          ...
          if (
            ...
            if (pending && !reencryptChangesets && newChangesetsSymmetricKey) {
              toEncrypt.push([
                [
                  "users",
                  userId,
  ...
  return {
    ...
    newChangesetKeysByEnvironmentId,
