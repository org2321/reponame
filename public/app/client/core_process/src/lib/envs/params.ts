...
import { encryptSymmetricWithKey } from "@core/lib/crypto";
import {
  getEnvInherits,
  getKeyableEnv,
...
export const envParamsForEnvironments = async (params: {
  ...
  for (let environmentId of environmentIds) {
    ...
    if (reencryptChangesets) {
      const changesets = (state.changesets[environmentId] ?? []).flatMap(
        R.prop("changesets")
      );
        ...
        for (let changesetId in byId) {
          ...
          toEncrypt.push([
            [...blobBasePath, "changesetsById", changesetId, "data"],
            {
              ...
              encryptionKey: reencryptChangesetKeysById[changesetId],
            },
          ]);
        }
      }
      ...
      toEncrypt.push([
        [...blobBasePath, "changesets"],
        {
          ...
          encryptionKey: newChangesetKeysByEnvironmentId[environmentId],
        },
      ]);
    }
  ...
  for (let [path, data] of pathResults) {
    blobs = R.assocPath(path, data, blobs);
  }
