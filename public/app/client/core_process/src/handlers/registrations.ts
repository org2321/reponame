...
import { generateKeys, signJson } from "@core/lib/crypto";
...
>({
  ...
  handler: async (
    ...
    if (payload.hostType === "cloud" || payload.devOnlyLocalSelfHosted) {
      ...
      if (apiRegisterRes.success) {
        ...
        failureAction = apiRegisterRes.resultAction as Client.Action.FailureAction;
        return dispatchFailure(
          failureAction.payload as Api.Net.ErrorResult,
          context
        );
      }
    } // end CLOUD / DEV-ONLY LOCAL SELF-HOSTED
