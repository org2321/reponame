...
export const handler = async (
  ...
  if (argv.remove) {
    if (state.requiresPassphrase) {
      const accountsRequiringPassphrase = (Object.values(
        state.orgUserAccounts
      ) as Client.ClientUserAuth[]).filter(R.prop("requiresPassphrase"));
    ...
    if (res.success) {
      console.log(chalk.bold("Passphrase set successfully."));

      console.log("");
      printDeviceSettings(res.state);
    } else {
      return exit(1, chalk.bold("Oops, there was a problem setting your passphrase."));
    }
  }
