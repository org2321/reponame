...
export const desc = "Minutes of inactivity before EnvKey locks on this device.";
...
export const handler = async (
  ...
  const { state } = await initCore(argv, false),
    accountsRequiringLockout = (Object.values(
      state.orgUserAccounts
    ) as Client.ClientUserAuth[]).filter(R.prop("requiresLockout"));
  ...
  const lowestMaxLockoutMs =
    ...
    accountsWithMin =
      typeof lowestMaxLockoutMs == "number"
        ? (Object.values(
            state.orgUserAccounts
          ) as Client.ClientUserAuth[]).filter(
  ...
  if (res.success) {
    console.log(chalk.bold("Lockout set successfully."));
    printDeviceSettings(res.state);
  } else {
