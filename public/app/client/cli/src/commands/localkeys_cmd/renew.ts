...
export const command = ["renew [app] [key-name]"];
...
export const handler = async (
  ...
  if (!authz.canGenerateKey(state.graph, auth.userId, localKey.id)) {
    return exit(
      1,
      chalk.red.bold("You do not have permission to renew the local key!")
    );
  }
  ...
  const table = new Table(); // do not constrain columns as this prevents entire key from being printed
  ...
  console.log(
    chalk.bold(
      "Local key renewed! It will not be shown again, so be sure to save it somewhere safe."
    )
  );
