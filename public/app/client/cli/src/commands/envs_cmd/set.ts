...
export const command = "set [app-or-block] [environment] [key-value-pairs...]";
export const desc = "Set app environment config values.";
export const builder = (yargs: Argv<BaseArgs>) =>
  yargs
    ...
    .option("force", {
      ...
      implies: "commit",
      describe:
        "Auto-confirm when using --commit flag with previous changes already staged",
    })
    .option("message", {
      type: "string",
      alias: "m",
...
export const handler = async (
  ...
  if (argv["sub-environment"]) {
    const name = argv["sub-environment"];
    if (environment) {
      ...
      environment = subEnvironmentsByName[name];
    } else {
      const environments = environmentsByName[name.toLowerCase()] ?? [];
      if (environments.length == 1) {
        environment = environments[0];
  ...
  if (argv["local-override"]) {
    ...
    if (
      ...
      return exit(
        1,
        chalk.red.bold(
          "You do not have permission to change the environment for that user."
        )
      );
    }
  } else {
    ...
    if (!environment) {
      return exit(
        1,
        chalk.red("Environment is not found, or you do not have access.")
      );
    }
  }
  ...
  if (argv["key-value-pairs"]) {
    kvArgs = kvArgs.concat(argv["key-value-pairs"]);
  }
  ...
  if (
    ...
    return exit(
      1,
      chalk.red.bold(
        `You do not have permission to modify the environment ${chalk.bold(
          envIdOrUserOverride
        )}.`
      )
    );
  ...
  for (let k of Object.keys(kv)) {
    ...
    if (val?.toLowerCase().startsWith("inherits:")) {
      ...
      if (inheritsName in environmentsByName) {
        const [inheritsEnvironment] = environmentsByName[inheritsName];
  ...
  if (argv.commit) {
      ...
      if (!argv.force) {
        if (hasPendingConflicts(state, [envParent.id], [envIdOrUserOverride])) {
          ...
          const { confirm } = await prompt<{ confirm: boolean }>({
            type: "confirm",
            name: "confirm",
            message: chalk.bold(`Commit all updates?`),
          });
    ...
    console.log(chalk.green("The changes have been committed successfully."));
    ...
    if (filteredPending) {
      console.log(
        chalk.bold("Your updates are pending:"),
        "\n" + filteredSummary,
        "\n" + filteredPending
      );
    } else {
