...
export const builder = (yargs: Argv<BaseArgs>) =>
  yargs
    ...
    .option("all", {
      ...
      alias: "a",
      describe:
        "Commit all pending changes across apps, blocks, environments, and overrides",
      conflicts: [
        "app-or-block",
        "environment",
        "local-override",
...
export const handler = async (
  ...
  const shouldCommitAll = argv.all;
  ...
  if (!pending) {
    console.log(
      chalk.bold("No pending updates for the parameters. No work was done.")
    );
  ...
  if (envParent && environment) {
    ...
    console.log(
      `Success! Use ${chalk.bold(
        "envkey show [app-or-block] [environments...]"
