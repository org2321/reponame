...
export const handler = async (
  ...
  if (!appChoices.length) {
    console.log(
      "You do not have access to any apps for which environments can be updated."
    );
    return exit();
  }
  if (argv["app"]) {
  ...
  if (!app) {
    ...
    if (appId) {
      const envFirst = firstArgIsEnvironment(state.graph, appId, argv["app"]);
  ...
  if (!environmentChoices) {
    return exit(
      1,
      chalk.red.bold(
        "You do not have permission to update any environments for the app."
      )
    );
  }
