...
export const command = ["destroy-org"];
export const desc = "Delete the current organization (danger!)";
...
export const handler = async (argv: BaseArgs): Promise<void> => {
  ...
  if (!authz.canDeleteOrg(state.graph, auth.userId)) {
    return exit(
      1,
      chalk.red.bold("You do not have permission to delete the organization.")
    );
  }
