...
export const handler = async (
  ...
  if (!authz.canCreateServer(state.graph, auth.userId, appEnv.id)) {
    return exit(
      1,
      chalk.red(
        "You do not have permission to create a server for this environment."
      )
    );
  }
  ...
  const table = new Table({
    // do not cap width or the key can be cut off :)
  });
  ...
  console.log(
    chalk.bold(
      "Server generated! It will not be shown again, so be sure to save it somewhere safe."
    )
  );
