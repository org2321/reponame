...
export const command = ["delete [app]", "rm [app]"];
...
export const builder = (yargs: Argv<BaseArgs>) =>
  yargs
    ...
    .option("force", {
      type: "boolean",
      alias: "f",
      describe: "Delete the app without a prompt for confirmation",
    });
export const handler = async (
  argv: BaseArgs & { app: string | undefined; force: boolean | undefined }
): Promise<void> => {
  ...
  if (!authz.canDeleteApp(state.graph, auth.userId, app.id)) {
    return exit(
      1,
      chalk.red.bold("You do not have permission to delete the app!")
    );
  }
