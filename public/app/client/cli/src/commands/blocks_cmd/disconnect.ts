...
export const desc = "Remove a block from an app.";
...
export const handler = async (
  ...
  if (!blockChoices.length) {
    return exit(
      1,
      chalk.red.bold(
        "There are no blocks available for you to connect to this app, or you do not have permission."
      )
    );
  }
  ...
  if (!existingAppBlock) {
    console.log(
      chalk.bold(`The app and block are not connected. No work was done.`)
    );
    return exit();
  }
  if (
    ...
    return exit(
      1,
      chalk.red.bold("You do not have permission to connect the app and block!")
    );
  }
  ...
  console.log(
    chalk.bold("The block and app have been disconnected successfully.")
  );
