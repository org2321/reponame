...
import { detectAppFromEnv } from "../envkey_detection";
...
export const authenticate = async (
    ...
    argv["detectedEnvkey"] = await detectAppFromEnv(state, argv, process.cwd());
  ...
  chooseAccount = async <IncludePendingSelfHostedType extends boolean>(
    ...
    let choices = (Object.values(
      ...
      .filter(({ token }) => !signedInOnly || token)
      .map((acct) => ({
        name: acct.userId,
    ...
    return (includePendingSelfHosted
      ? state.orgUserAccounts[id] ??
        state.pendingSelfHostedDeployments[parseInt(id)]
      : state.orgUserAccounts[id]) as IncludePendingSelfHostedType extends true
  ...
  chooseOrg = async (
    ...
    const { id } = await prompt<{ id: string }>({
      ...
      choices: (Object.values(state.orgUserAccounts) as Client.ClientUserAuth[])
        .filter((acct) => (acct.email == emailOrUserId || acct.userId === emailOrUserId) && (!signedIn || acct.token))
        .map((acct) => ({
          name: "id",
          value: acct.userId,
  ...
  authFromEmail = async (
    ...
    const accounts =
      ...
      candidates = accounts.filter((acct) => acct!.email == emailOrUserId || acct.userId == emailOrUserId);
    ...
    switch (auth.provider) {
      case "email":
        ...
        ({ emailVerificationToken } = await prompt<{
          ...
          message: `An email verification was just sent to ${email}. Please paste it here:`,
        }));
