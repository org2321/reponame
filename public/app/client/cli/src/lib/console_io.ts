...
let _autoModeWroteOnce = false;

export const isAutoMode = (): boolean => Boolean(_autoModeStatus);

export const autoModeHasWritten = (): boolean => Boolean(_autoModeWroteOnce);
...
// Auto-Mode disables the normal CLI user-friendly console output, both stdout and stderr. It also
...
const disablePromptsFunc = () => {
  ...
  throw new Error(
    "This command has input prompts, and there are missing arguments or flags. Try running the command without auto-mode enabled, and see which values are requested. Then add the corresponding positional or flag arguments."
  );
};
