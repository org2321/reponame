...
export const findUser = (
  ...
  if (!res && deletedGraph) {
    const { orgUsers: deletedOrgUsers, cliUsers: deletedCliUsers } = graphTypes(
      deletedGraph
    );
...
export const findApp = (
  ...
  graphTypes(graph).apps.find((a) =>
    [a.name.toLowerCase(), a.id.toLowerCase()].includes(nameOrId.toLowerCase())
  );
...
export const appsConnectBlockMustValidate = (
  ...
  if (!app) {
    let message = chalk.red.bold(
      `The app ${appName} does not exist, or you do not have access.`
    );
  ...
  if (!block) {
    let message = chalk.red.bold(
      `The block ${blockName} does not exist, or you do not have access.`
    );
...
export const requireUserAppRoleAndGrant = (
  ...
  const existingAppUserGrant = getAppUserGrantsByComposite(graph)[
    [userId, appId].join("|")
  ];
  if (!existingAppUserGrant) {
    console.log(
      `${chalk.bold(user.email)} has access to ${
        app.name
      } implied via their org role. No work was done.`
    );
    return exit();
  }
...
export const mustSelectSubEnvironmentForDeletion = async (
  ...
  const appOrBlock = graph[envParentId] as Model.EnvParent;
  ...
  if (parentEnvName) {
    const p = findEnvironment(graph, envParentId, parentEnvName);
    if (!p || p.isSub) {
    ...
    allSubEnvs = allEnvs.filter(
      (e) =>
        e.isSub &&
        (e.parentEnvironmentId === parentEnvName ||
  ...
  if (!allSubEnvs.length) {
    return exit(
      1,
      chalk.red.bold(
        `No sub-environments exist for the ${appOrBlock.type}${
          parentEnvName ? " and parent" : ""
        }!`
      )
    );
  }
  ...
  const promptEnvironment = () =>
    prompt<{ environment: string }>({
      type: "select",
      name: "environment",
      message: "Select sub-environment:",
      initial: 0,
      required: true,
      choices: R.sortBy(
        R.prop("message"),
  ...
  const subEnvsChosen = allSubEnvs.filter(
    (env) =>
      ...
      (env.id === subEnvNameOrId ||
        getEnvironmentName(graph, env.id).toLowerCase() ===
          subEnvNameOrId.toLowerCase())
  ) as Model.Environment[];
  if (!subEnvsChosen.length) {
    return exit(
      1,
      chalk.red(
        `Sub-environment ${chalk.bold(subEnvNameOrId)} does not exist${
          parentEnvName ? " for parent " + chalk.bold(parentEnvName) : ""
  ...
  if (!subEnv) {
    return exit(1, chalk.red(`Sub-environment not found!`));
  }
...
> => {
  ...
  let appOrBlockArg = argv["app-or-block"];
  let environmentArg = argv["environment"];
  let shiftedPositional = argv["argvThirdPositional"];

  if (appOrBlockArg) {
    appOrBlock =
      findApp(state.graph, appOrBlockArg) ??
      findBlock(state.graph, appOrBlockArg);
  }
  ...
  if (!appOrBlock) {
    ...
    if (!envParentChoices.length) {
      console.log(
        `You do not have access to any apps or blocks for this action.`
      );
      return exit();
    }
    const appOrBlockName = (
      await prompt<{ appOrBlockName: string }>({
        type: "select",
        name: "appOrBlockName" + "",
        message: "App/Block:",
        choices: envParentChoices,
      })
    ).appOrBlockName as string;
    appOrBlock =
      findApp(state.graph, appOrBlockName) ||
      findBlock(state.graph, appOrBlockName);
  }
  ...
  if (argv["local-override"]) {
    localOverrideEnvironmentId = [appOrBlock.id, auth.userId].join("|");
    return {
      auth,
      state,
      shiftedPositional,
  ...
  if (argv["override-user"]) {
    ...
    if (
      ...
      return exit(
        1,
        chalk.red.bold("You do not have override permissions for that user.")
      );
    }
    localOverrideEnvironmentId = [appOrBlock.id, otherUser.id].join("|");
    return {
      state,
      auth,
      shiftedPositional: 0,
  ...
  if (!environmentChoices) {
    return exit(
      1,
      chalk.red(
        `You do not have access to perform this action for the environments under ${chalk.bold(
          appOrBlock.name
        )}.`
      )
    );
  ...
  if (environmentArg && argv["sub-environment"]) {
    appEnv = graphTypes(state.graph)
      // find sub-environment
      .environments.filter((e) => e.envParentId === appOrBlock!.id && e.isSub)
      .find((e) => {
        ...
        return (
          ...
          findEnvironment(state.graph, e.envParentId, argv["sub-environment"]!)
        );
      });
    if (!appEnv) {
      return exit(
        1,
        chalk.red(
          `The sub-environment does not exist, or it did not match the parent environment for the ${chalk.bold(
            appOrBlock.type
          )}.`
        )
      );
    ...
    appEnv = findEnvironment(state.graph, appOrBlock.id, environmentName);
  }
  if (!appEnv) {
