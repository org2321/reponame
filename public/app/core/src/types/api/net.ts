...
export namespace Net {
  ...
  const FetchEnvsParamsSchema = utils.intersection(
    z.object({
      envParentIds: z.array(z.string()),
    }),
    z.union([
      z.object({
        envs: z.literal(true),
        changesets: z.literal(true),
        changesetOptions: FetchChangesetOptionsSchema.optional(),
      }),
      z.object({
        envs: z.literal(true),
        changesets: z.undefined(),
        changesetOptions: z.undefined(),
      }),
      z.object({
        changesets: z.literal(true),
        envs: z.undefined(),
  ...
  type EnvsResult = {
    envs: {
      ...
      blobs: Blob.UserEncryptedBlobsByEnvironmentIdOrComposite;
    };
  };
  ...
  export type GraphWithEnvs = EnvsResult & {
    type: "graphWithEnvs";
    graph: Client.Graph.UserGraph;
    graphUpdatedAt: number;
    timestamp: number;
  ...
  export type LoadedInvite = Omit<GraphWithEnvs, "type"> & {
    type: "loadedInvite";
    orgId: string;
    invite: Pick<
      Api.Db.Invite,
  ...
  export type LoadedDeviceGrant = Omit<GraphWithEnvs, "type"> & {
  ...
  export type LoadedRecoveryKey = Omit<GraphWithEnvs, "type"> & {
  ...
  const registerSchema = utils.intersection(
    z.object({
      ...
      org: Model.OrgSchema.pick({ name: true, settings: true }),
    }),
    z.union([
      utils.intersection(
        z.object({
      ...
      z.object({
        ...
        provider: z.literal("email"),
        emailVerificationToken: z.string().optional(), // for local self-hosted
        domain: z.string(),
      }),
    ])
  );
  ...
  export type ApiResultTypes = {
    ...
    GetExternalAuthProviders: {
      ...
      samlSettingsByProviderId:
        | Record<string, Model.SamlProviderSettings | undefined>
        | undefined;
    };
    ...
    CreateOrgSamlProvider: Model.ExternalAuthProvider;
  ...
  export type LargeObjectResult = {
    type: "largeObjectResult";
    url: string;
  };
