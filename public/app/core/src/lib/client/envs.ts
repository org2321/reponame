...
export const getEnvWithMeta = (
  ...
  getPendingEnvWithMeta = memoize(
    ...
    ) => getEnvWithMeta(state, params, true)
  ),
  getPendingActions = (
    state: Client.State,
    params: {
  ...
  getPendingActionsByEnvironmentId = memoize((state: Client.State) =>
    R.groupBy((action) => action.meta.environmentId, state.pendingEnvUpdates)
  ),
  ...
  getInheritanceChain = (
    ...
    const keyableEnv = getKeyableEnv(state, params, pending);
    let currentEnvironmentId = keyableEnv[params.key]?.inheritsEnvironmentId;
  ...
  getDiffsByKey = (
    ...
    for (let k of allKeys) {
      ...
      if (k in fromVars && !(k in toVars)) {
        byKey[k] = {
          fromValue: fromVars[k],
          toValue: undefined,
        };
      } else if (k in toVars && !(k in fromVars)) {
        byKey[k] = {
          fromValue: undefined,
          toValue: toVars[k],
        };
  ...
  ensureEnvsFetched = (state: Client.State, envParentId: string) => {
    if (envsNeedFetch(state, envParentId)) {
      ...
      const msg = `latest envs not fetched for ${envParent.name}`;
      console.log(msg);
      throw new Error(msg);
    }
  },
  ensureChangesetsFetched = (state: Client.State, envParentId: string) => {
    if (changesetsNeedFetch(state, envParentId)) {
      const envParent = state.graph[envParentId] as Model.EnvParent;
      const msg = `latest changesets not fetched for ${envParent.name}`;
      console.log(msg);
      throw new Error(msg);
    }
  },
  ...
  changesetsNeedFetch = (state: Client.State, envParentId: string) => {
    ...
    for (let [environmentId, decrypted] of R.toPairs(state.changesets)) {
