...
export const getAppBlockGroupAssoc = memoize(
    (graph: Graph.Graph, appId: string, blockId: string) => {
      ...
      const appBlockGroup = blockGroupIds
        .map(
          (blockGroupId) =>
            getAppBlockGroupsByComposite(graph)[[appId, blockGroupId].join("|")]
        )
        .filter(Boolean)[0];
      ...
      const appGroupBlock = appGroupIds
        .map(
          (appGroupId) =>
            getAppGroupBlocksByComposite(graph)[[appGroupId, blockId].join("|")]
        )
        .filter(Boolean)[0];
  ...
  getConnectedAppsForBlock = memoize((graph: Graph.Graph, blockId: string) => {
    ...
    return apps.filter(
      ({ id: id }) =>
        getAppBlocksByComposite(graph)[[id, blockId].join("|")] ??
  ...
  getConnectedBlocksForApp = memoize((graph: Graph.Graph, appId: string) => {
    const blocks = graphTypes(graph).blocks,
      connected = R.sortBy(
        ...
        blocks.filter(
          ({ id: id }) =>
            getAppBlocksByComposite(graph)[[appId, id].join("|")] ??
  ...
  getBlockSortVal = (graph: Graph.Graph, appId: string, blockId: string) => {
    const assoc = getAppBlockGroupAssoc(graph, appId, blockId),
      blockGroupMembership = assoc
        ...
        : undefined,
      appBlock = getAppBlocksByComposite(graph)[[appId, blockId].join("|")];
  ...
  getFirstApp = memoize((graph: Client.Graph.UserGraph) => {
    const firstApp = R.sortBy(R.prop("name"), graphTypes(graph).apps)[0] as
      | Model.App
      | undefined;
    return firstApp;
  }),
  getFirstAppWithConnectedBlockIds = memoize(
    (graph: Client.Graph.UserGraph) => {
      const firstApp = getFirstApp(graph);
      if (firstApp) {
        return getEnvParentWithConnectedIds(graph, firstApp.id);
      } else {
        return [];
      }
    }
  ...
  getConnectedBlockEnvironmentsForApp = memoize(
    (
      ...
      if (environmentId) {
        appEnvironments = appEnvironments.filter(R.propEq("id", environmentId));
      }

      const appEnvironmentComposites = appEnvironments.map(
          environmentCompositeId
        ),
      ...
      const blockEnvironmentsByComposite = R.groupBy(
        environmentCompositeId,
        blockEnvironments
      );
      const res = R.sortBy(
        ...
        R.flatten(
          Object.values(
            R.pick(appEnvironmentComposites, blockEnvironmentsByComposite)
          )
        )
      );
  ...
  getConnectedAppEnvironmentsForBlock = memoize(
    (
      ...
      const environmentsByEnvParentId = getEnvironmentsByEnvParentId(graph),
        ...
        blockEnvironmentsByComposite = R.indexBy(
          environmentCompositeId,
          blockEnvironments
        );
