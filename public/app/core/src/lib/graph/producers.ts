    ...
    (graphDraft: Draft<T>) => {
      ...
      const byType = graphTypes(graph),
        authObjects: {
          ...
          R.groupBy(R.prop("inviteeId"), byType.invites),
          R.groupBy(R.prop("granteeId"), byType.deviceGrants),
        ],
                  ...
                  (objects) =>
                    R.tail(
                      R.sortBy(
                        ({ expiresAt }) => -expiresAt,
                        objects.filter(
                          ({ acceptedAt, expiresAt, deletedAt }) =>
    ...
    (graphDraft) => {
      ...
      if (
        ...
        graphTypes(graphDraft).apps.forEach((app) => {
          const existingEnvironmentForRole = R.indexBy(
