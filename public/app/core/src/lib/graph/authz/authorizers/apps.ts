...
export const canCreateApp = (graph: Graph.Graph, currentUserId: string) =>
  ...
  canGrantAppRoleToOrgRole = (
    ...
    if (!targetAppRole.defaultAllApps) {
      const includedAppRole = getIncludedAppRolesByComposite(graph)[
  ...
  canGrantAppRoleToUserGroup = (
    ...
    // make sure we can grant this app role to the org role of all members
    // of this group
    const memberships =
      getGroupMembershipsByObjectId(graph)[params.appId] ?? [];
    for (let { objectId } of memberships) {
      const { orgRoleId } = graph[objectId] as Model.OrgUser;
      if (
        !canGrantAppRoleToOrgRole(graph, currentUserId, {
          appId: params.appId,
          appRoleId: params.appRoleId,
  ...
  canRemoveAppUserAccess = (
    ...
    const appUserGrantId =
      "appUserGrantId" in params
        ...
        : getAppUserGrantsByComposite(graph)[
            [params.userId, params.appId].join("|")
          ]?.id;
