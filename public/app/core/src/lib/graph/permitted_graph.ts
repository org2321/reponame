...
export const getPermittedGraphObjects = (
  ...
  const [permittedOrgUserDevices, permittedDeviceGrants, permittedInvites] = [
    ...
    permittedFiles = byType.files.filter(
      ({ envParentId }) =>
        permittedAppIds.has(envParentId) || permittedBlockIds.has(envParentId)
    ),
  ...
  if (
    ...
    currentOrgPermissions.has("org_manage_user_groups") ||
    ...
    if (currentOrgPermissions.has("org_manage_user_groups")) {
