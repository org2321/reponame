...
export const getOrgPermissions = memoize(
  ...
  getEnvironmentPermissions = memoize(
    (
        ...
        if (orgPermissions.has("blocks_write_envs_all")) {
          ...
          if (orgPermissions.has("blocks_read_all")) {
            ...
            if (connectedApps.length > 0) {
              const environmentsByEnvParentId = getEnvironmentsByEnvParentId(
                graph
              );
      ...
      if (environment.isSub) {
        if (permissionSet.has("write") && !permissionSet.has("write_subenvs")) {
          permissionSet.delete("write");
        }

        if (permissionSet.has("read") && !permissionSet.has("read_subenvs")) {
          permissionSet.delete("read");
        }

        if (
          permissionSet.has("read_meta") &&
          !permissionSet.has("read_subenvs_meta")
        ) {
        ...
        if (
          ...
          !permissionSet.has("read_subenvs_inherits")
        ) {
        ...
        if (
          ...
          !permissionSet.has("read_subenvs_history")
        ) {
      ...
      for (let permission of permissionSet) {
        ...
        if (environment.isSub) {
          if (permission == "read_subenvs") v = "read";
          if (permission == "read_subenvs_history") v = "read_history";
          if (permission == "write_subenvs") v = "write";
          if (permission == "read_subenvs_inherits") v = "read_inherits";
          if (permission == "read_subenvs_meta") v = "read_meta";
        }
        if (!v) {
          v = permission;
        }
  ...
  getAppRoleEnvironmentRolePermissions = (
    ...
    if (appRole.isDefault && environmentRole.isDefault) {
      ...
      const appRoleEnvironmentRole = getAppRoleEnvironmentRolesByComposite(
        graph
      )[[appRoleId, environmentRoleId].join("|")];
  ...
  getAppRoleForUserOrInvitee = memoize(
    (
      ...
      if (orgRole.autoAppRoleId) {
        ...
        const appUserGrant = getAppUserGrantsByComposite(graph)[
          [userId, appId].join("|")
        ];
        if (appUserGrant) {
  ...
  getAppRoleForUserGroup = memoize(
    (graph: Graph.Graph, appId: string, userGroupId: string) => {
      const appUserGroup = getAppUserGroupsByComposite(graph)[
        [appId, userGroupId].join("|")
      ];
      ...
      const appGroupUserGroupsByComposite = getAppGroupUserGroupsByComposite(
        graph
      );
      for (let appGroupId of appGroupIds) {
        const appGroupUserGroup =
          appGroupUserGroupsByComposite[[appGroupId, userGroupId].join("|")];
