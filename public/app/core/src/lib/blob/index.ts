...
export const userEncryptedKeyPkey = (params: Blob.UserEncryptedKeyPkeyParams) =>
    [...R.props(["orgId", "userId", "deviceId"], params), "encryptedKeys"]
      .filter(Boolean)
      .join("|"),
  getSkeyOrScope = (params: Blob.ScopeParams | Blob.SkeyParams) => {
    ...
    if (params.blobType == "userMessage") {
      if ("envParentId" in params && params.envParentId) {
        path.push(params.envParentId);
      } else if ("userId" in params && params.userId) {
        path.push(params.userId);
      } else if ("userGroupId" in params && params.userGroupId) {
        path.push(params.userGroupId);
      }
    } else if (params.blobType == "credential") {
      if (params.envParentId) {
        path.push(params.envParentId);
      }

      if (params.credentialId) {
        path.push(params.credentialId);
      }
    } else if (params.envParentId) {
      path.push(params.envParentId);

      if (params.blobType == "env" || params.blobType == "file") {
      ...
      if (params.blobType == "changeset" && "id" in params) {
        path.push(params.id);
      }
    }
  ...
  encryptedBlobPkey = (params: Blob.EncryptedBlobPkeyParams) =>
    [params.orgId, "encryptedBlobs"].join("|"),
      ...
      for (let userId in keySet.users) {
        for (let deviceId in keySet.users[userId]) {
          ...
          for (let path of filtered) {
            filteredKeySet = R.assocPath(
              ["users", userId, deviceId, ...path],
              true,
              filteredKeySet
            );
          }
    ...
    if (keySet.keyableParents) {
      for (let keyableParentId in keySet.keyableParents) {
        ...
        if (
          ...
          filteredKeySet = R.assocPath(
            ["keyableParents", keyableParent.id],
            true,
            filteredKeySet
          );
        }
