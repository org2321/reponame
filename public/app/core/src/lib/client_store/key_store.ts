...
const getKeytar: () => typeof Keytar = () => {
  ...
  if (typeof __non_webpack_require__ !== "undefined") {
    ...
    if (process.env.ENVKEY_CLI_BUILD_VERSION) {
      try {
        maybeLog("loading keytar from ./", { __dirname });
        _keytar = __non_webpack_require__("./keytar.node"); // deployed to the same folder
        maybeLog("keytar was loaded from ./", { __dirname });
        return _keytar;
      } catch (loadErr) {
        maybeLog("did not load keytar from ./", {
          loadErr,
        });
      }
    }
  ...
  try {
    ...
    maybeLog("keytar was loaded from 'keytar'", { _keytar });
    return _keytar;
  } catch (loadErr) {
    maybeLog("did not load keytar from 'keytar'", {
      loadErr: loadErr.message,
    });
  }
...
export const initKeyStore = async () => {
  ...
  getCoreProcAuthToken = async () => {
    ...
    encryptedAuthTokenPromise = encryptSymmetricWithKey({
