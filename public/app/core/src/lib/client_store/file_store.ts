...
import { encryptSymmetricWithKey, decryptSymmetricWithKey } from "../crypto";
...
export const put = async (k: string, v: string | {}) => {
  ...
  get = async (k: string) => {
    ...
    if (!("key" in deviceKey && deviceKey.key)) {
      // no-op when key store is locked
      if (loggingEnabled) log(`Couldn't fetch ${k}: device key locked. No-op.`);
      return null;
    }
    ...
    try {
      ...
      if (loggingEnabled) log(`Fetched and decrypted ${k}.`);
  ...
  del = (k: string) => {
    if (loggingEnabled) log(`Deleting ${k} from file store...`);
    fileStore.delete(k);
    if (loggingEnabled) log(`${k} deleted.`);
  },
