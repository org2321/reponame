...
export const authenticate = async <
  ...
  >(
    ...
    switch (params.type) {
      case "tokenAuthParams":
        ...
        [authToken, user, org, orgUserDevice] = await Promise.all([
          getAuthToken(
            params.orgId,
            params.userId,
            params.deviceId,
          ...
          getOrgUser(params.orgId, params.userId),
          getOrg(params.orgId),
          getDb<Api.Db.OrgUserDevice>(
            graphKey(params.orgId, "orgUserDevice", params.deviceId)
          ),
        ]);
        break;
      case "cliAuthParams":
        [user, org] = await Promise.all([
          getDb<Api.Db.CliUser>(
            graphKey(params.orgId, "cliUser", params.userId)
          ),
          getOrg(params.orgId),
        ]);
        break;
      ...
      case "acceptInviteAuthParams":
        ...
        const invitePointer = await getDb<Api.Db.InvitePointer>({
          pkey: sha256(params.identityHash),
        ...
        [org, invite] = await Promise.all([
          getOrg(invitePointer.orgId),
          getDb<Api.Db.Invite>(
            graphKey(invitePointer.orgId, "invite", invitePointer.inviteId)
          ),
        ]);
        ...
        user = await getOrgUser(invitePointer.orgId, invite.inviteeId);
        break;
      ...
      case "acceptDeviceGrantAuthParams":
        ...
        [org, deviceGrant] = await Promise.all([
          getOrg(deviceGrantPointer.orgId),
          getDb<Api.Db.DeviceGrant>(
            graphKey(
              deviceGrantPointer.orgId,
              "deviceGrant",
              deviceGrantPointer.deviceGrantId
            )
          ),
        ]);
      ...
      case "redeemRecoveryKeyAuthParams":
        ...
        [org, recoveryKey] = await Promise.all([
          getOrg(recoveryKeyPointer.orgId),
          getDb<Api.Db.RecoveryKey>(
            graphKey(
              recoveryKeyPointer.orgId,
              "recoveryKey",
              recoveryKeyPointer.recoveryKeyId
            )
          ),
        ]);
        ...
        user = await getOrgUser(recoveryKeyPointer.orgId, recoveryKey.userId);
        break;
      ...
      case "bearerTokenAuthParams":
        ...
        org = await getOrg(provisioningProvider.orgId);
        if (!org || org?.deletedAt) {
          throw new Api.ApiError("Provider org found", 404);
        }
  ...
  verifyEmailToken = async (email: string, token: string, now: number) => {
    const emailVerification = await getActiveEmailVerification(email, token);
    if (
      !emailVerification ||
  ...
  getExternalAuthSession = async (
    ...
    const externalAuthSession = await query<Api.Db.ExternalAuthSession>({
      pkey: externalAuthSessionId,
      limit: 1,
    });
    if (externalAuthSession?.[0]?.type !== "externalAuthSession") {
      return undefined;
    }
    return externalAuthSession[0];
  },
  verifyExternalAuthSession = async (externalAuthSessionId: string) => {
...
const authorizeEnvUpdate = (
  ...
  authorizeKeyableParentUpdate = (
    ...
    if (update.subEnv) {
      if (!currentUserPermissions.has("write_subenvs")) {
        return false;
      }
    }
