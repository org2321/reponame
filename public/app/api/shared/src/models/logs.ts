...
import { Model, Auth, Api, Client, Logs, Rbac, Blob } from "@core/types";
...
export const fetchLogs = async (
    ...
    const [logs, deletedGraph, ips] = await Promise.all([
      ...
      params.pageNum == 0
        ? getDeletedGraph(auth, orgGraph, startsAt, endsAt)
  ...
  getDeletedGraphForRange = async (
    ...
    return getDeletedGraph(auth, orgGraph, startsAt, endsAt);
  },
...
const getIps = async (orgId: string, startsAt: number, endsAt: number) => {
  ...
  getFetchLogsTransactionIdsResults = async (
    table: "transaction_ids" | "transaction_ids_by_target_id",
    orgId: string,
    params: Logs.FetchLogParams,
    startsAt: number,
    endsAt: number,
    ...
    let targetUserDeviceConditions: string[] | undefined;
    let targetUserDeviceIds: string[] | undefined;
    let targetUserDeviceQargs: any[] | undefined;

    if (table == "transaction_ids") {
      if (params.userIds) {
        targetUserDeviceConditions = ["t.actorId IN (?)"];
        targetUserDeviceQargs = [params.userIds];
      } else if (params.deviceIds) {
        targetUserDeviceConditions = ["t.deviceId IN (?)"];
        targetUserDeviceQargs = [params.deviceIds];
      }
    } else {
      if (params.targetIds) {
        if (params.userIds) {
          ...
          for (let userId of params.userIds) {
            for (let targetId of params.targetIds) {
              ...
              compoundIds.push([userId, targetId].join("||"));
            }
          }
          targetUserDeviceIds = compoundIds;
          targetUserDeviceQargs = [params.userIds, compoundIds];
        } else if (params.deviceIds) {
          targetUserDeviceConditions = [
            "t.deviceId NOT IN (?)",
            "t.targetId IN (?)",
          ...
          for (let deviceId of params.deviceIds) {
            for (let targetId of params.targetIds) {
              compoundIds.push([deviceId, targetId].join("||"));
            }
          }
          targetUserDeviceIds = compoundIds;
          targetUserDeviceQargs = [params.deviceIds, compoundIds];
        } else {
          targetUserDeviceConditions = ["t.targetId IN (?)"];
          targetUserDeviceIds = params.targetIds;
          targetUserDeviceQargs = [params.targetIds];
        }
        ...
        targetUserDeviceQargs = [params.userIds, params.userIds];
      } else if (params.deviceIds) {
        targetUserDeviceConditions = [
          "t.deviceId NOT IN (?)",
          "t.targetId IN (?)",
        ...
        targetUserDeviceQargs = [params.deviceIds, params.deviceIds];
      }
    }
    ...
    // log((await pool.getConnection()).format(qs, qargs));

    const [[rows], [[{ totalCount, countReachedLimit }]]] = (await Promise.all([
      pool.query(qs, qargs),
      params.pageNum == 0
        ? pool.query(countQs, qargs)
...
export const getLogTransactionStatement = ({
  ...
  responseBytes,
  now,
}: LogActionParams): Api.Db.SqlStatement => {
  let qs = "",
    qargs: any[] = [];
  ...
  if (previousOrgGraph && updatedOrgGraph) {
    addCompoundTargetIds(previousOrgGraph, updatedOrgGraph, loggedAction);
  }
  ...
  qs += `INSERT INTO logs (${mainFields.join(", ")}) VALUES (${R.repeat(
    "?",
    mainFields.length
  ).join(", ")});`;
  ...
  for (let field of mainFields) {
    if (field == "body") {
      qargs.push(JSON.stringify(R.omit(mainFields, loggedAction)));
    } else {
      qargs.push(loggedAction[field] ?? null);
    }
  }

  qs += `SET ${transactionIdFields
    .map((field) => `@${field} = ?`)
  ...
  if ("targetIds" in loggedAction) {
    for (let targetId of loggedAction.targetIds) {
      qs += `SET ${byTargetIdFields
        .map((field) => `@${field} = ?`)
        .join(
          ", "
  ...
  qs += `SET @now = ?; INSERT INTO ips (ip, orgId, createdAt, lastRequestAt) VALUES (?, ?, @now, @now) ON DUPLICATE KEY UPDATE lastRequestAt = @now;`;
  qargs.push(now, loggedAction.ip, auth?.org.id ?? "");
...
const addCompoundTargetIds = (
  previousOrgGraph: Api.Graph.OrgGraph,
  updatedOrgGraph: Api.Graph.OrgGraph,
  loggedAction: Logs.LoggedAction
) => {
  if (!("targetIds" in loggedAction) || loggedAction.targetIds.length == 0) {
    return;
  }
  ...
  for (let userTargetId of userTargetIds) {
    for (let otherTargetId of otherTargetIds) {
      loggedAction.targetIds.push([userTargetId, otherTargetId].join("||"));
    }
  }

  for (let deviceTargetId of deviceTargetIds) {
    for (let otherTargetId of [...otherTargetIds, ...localsTargetIds]) {
      loggedAction.targetIds.push([deviceTargetId, otherTargetId].join("||"));
    }
  }
};
  ...
  */
  ...
  switch (action.type) {
    ...
    case Api.ActionType.UPGRADE_SELF_HOSTED:
      return "+upgrade+ Self-Hosted EnvKey";
    ...
    case Api.ActionType.LIST_INVITABLE_SCIM_USERS:
      return `+list+ invitable users from SCIM directory %${action.payload.id}%`;
    ...
    case Api.ActionType.FETCH_ENVS:
      ...
      if (action.payload.envs && action.payload.changesets) {
        fetchType = "environments and changesets";
      } else if (action.payload.envs) {
        fetchType = "environments";
      } else {
        fetchType = "changesets";
      }

      return `+fetch+ ${fetchType} for ${action.payload.envParentIds
        .map((id) => {
    ...
    case Api.ActionType.CREATE_ENVIRONMENT:
      ...
      return `+include+ environment %${action.payload.environmentRoleId}% in ${envParent.type} %${envParent.id}%`;
    ...
    case Api.ActionType.DELETE_ENVIRONMENT:
      ...
      return `+remove+ environment %${environment.environmentRoleId}% from ${envParent.type} %${envParent.id}%`;
...
export const getFetchActionLogTargetIdsFn =
  (orgGraph: Api.Graph.OrgGraph) => (response: Api.Net.ApiResult) => {
    const { envs, changesets } = response as Api.Net.GraphWithEnvs;
