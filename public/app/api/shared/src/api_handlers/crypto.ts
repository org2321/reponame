...
import { Api, Auth, Model, Graph } from "@core/types";
import {
  authz,
  deleteGraphObjects,
  graphTypes,
      ...
      for (let envParentId in blobs) {
        ...
        if (environments) {
          for (let environmentId of Object.keys(environments)) {
            delete (draft[environmentId] as Model.Environment)
              .reencryptionRequiredAt;
          }
        }
        if (locals) {
          for (let localsUserId of Object.keys(locals)) {
            delete (draft[envParentId] as Model.EnvParent)
              .localsReencryptionRequiredAt[localsUserId];
          }
        }
      }
    });
