...
import {
  parameterStoreDeploymentKey,
  getFargateStackName,
  PARAM_API_VERSION_NUMBER,
  PARAM_INFRA_VERSION_NUMBER,
...
export const preDeployValidations = async (
  ...
  const regionSettings = primaryRegionSettings[primaryRegion as Region];
  if (!regionSettings) {
    throw new Error(
      `Invalid primary region! Supported regions are: ${Object.keys(
        primaryRegionSettings
      )}`
    );
  }
  const { failoverRegion } = regionSettings;
  ...
  const failoverRegionCertificateArn = await getAwsCertArnForDomain(
    profile,
    failoverRegion,
    domain
  );
...
export const createBucketIfNeeded = (s3Instance: S3) => async (
  name: string
) => {
...
export const getSesAzsForRegion = async (
  profile: string | undefined,
  region: string
) => {
  // only certain AZs support ses email-smtp service, and these have changed within the same region
  ...
  const services = await ec2
    .describeVpcEndpointServices({
      ServiceNames: [`com.amazonaws.${region}.email-smtp`],
    })
    .promise();

  const azs = services.ServiceDetails?.[0].AvailabilityZones ?? [];
  if (azs.length < 3) {
    throw new Error(
      `Only regions with 3 or more availability zones with SES SMTP capacity are supported. Found: ${azs.length} ${azs}`
    );
  }
  return azs;
};
...
export const validateSenderEmail = async (
  ...
  if (!identities || !identities.length) {
    throw new Error(
      `No SES email sender identities were found for ${region}. ${blurb}`
    );
  }
  ...
  if (!identity) {
    throw new Error(
      `${blurb} Found ${identities.length} other sender identities in ${region}. ${identities}`
    );
  }
  ...
  if (identityVerification.VerificationStatus !== "Success") {
    throw new Error(
      `Found sender identity, but it is not yet verified by AWS.`
    );
  }
  // ok
};
...
export const processTemplatesReturningFolder = async (
  ...
  execSync(
    `unzip ${containingFolder}/${zipFileName} -d ${extractToFolder}`,
    { cwd: process.cwd() }
  );
...
export const getCurrentFargateEnvkeyVersions = async (params: {
  profile?: string;
  primaryRegion: string;
  ...
  const credentials = params.profile
    ...
    : undefined;
  const cf = new CF({ region: params.primaryRegion, credentials });
  let apiVersion = "0.0.0";
  let infraVersion = "0.0.0";

  try {
    const { Stacks } = await cf
      .describeStacks({
        StackName: getFargateStackName(params.deploymentTag),
      })
      .promise();
    const existingStack = Stacks![0]!;

    let av = existingStack.Parameters?.find(
      (p) => p.ParameterKey === PARAM_API_VERSION_NUMBER
    )?.ParameterValue;
    if (av) {
      apiVersion = av;
    }

    let iv = existingStack.Parameters?.find(
      (p) => p.ParameterKey === PARAM_INFRA_VERSION_NUMBER
    )?.ParameterValue;
    if (iv) {
      infraVersion = iv;
    }
  ...
  return { apiVersion, infraVersion };
};
