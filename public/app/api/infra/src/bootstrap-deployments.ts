...
export const bootstrapSelfHostedDeployment = async (
  ...
  await sns
    .subscribe({
      Endpoint: params.verifiedSenderEmail,
      Protocol: "email",
      TopicArn: topic.TopicArn!,
    })
    .promise();
  if (params.notifyEmailWhenDone) {
    await sns
      .subscribe({
        Endpoint: params.notifyEmailWhenDone,
        Protocol: "email",
        TopicArn: topic.TopicArn!,
      })
      .promise();
...
export const createUpdaterProject = async (params: {
  ...
  const environmentVariables: CodeBuild.EnvironmentVariables = [
    ...
    // of the Infra which is different than its published latest infra-version.txt on master.
